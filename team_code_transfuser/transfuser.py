...
class TransfuserBackbone(nn.Module):
    ...
    def get_next_frame_index(save_dir):
        """Return the next available frame index based on existing files."""
        os.makedirs(save_dir, exist_ok=True)
        existing = [f for f in os.listdir(save_dir) if f.startswith("frame_") and f.endswith(".png")]
        if not existing:
            return 0
         # Extract numeric parts
        indices = [int(re.findall(r"frame_(\d+).png", f)[0]) for f in existing]
        return max(indices) + 1

    def save_frame(image_tensor, save_dir="/mnt/iusers01/fse-ugpgt01/compsci01/f15583hs/scratch/video"):
        """
        Save a batch of images sequentially to disk.
        Only saves the first image in the batch for visualization.
        """
        next_idx = get_next_frame_index(save_dir)

        # Take first image in batch
        img = image_tensor[0].detach().cpu()

        # If normalized (ImageNet), unnormalize
        ...
        # Save sequentially
        frame_path = os.path.join(save_dir, f"frame_{next_idx:06d}.png")
